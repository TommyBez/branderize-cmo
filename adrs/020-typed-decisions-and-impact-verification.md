...
`IntentPreauthorizationDecision` deliberately does not contain an Intent revision. The proposal content may be prepared by a model, while the revision being authorized is trusted boundary state: `recordDecision` pins it as `authorized_intent_revision` in the producing `decision_recorded` Action and its Policy snapshot. The Decision's logical head remains revision-agnostic; applicability is a separate comparison between that trusted Action fact and the canonical current Intent revision.
...
All Decision Objects are brand-scoped in v1. An `intent_preauthorization` additionally requires the producing Action's exact same-brand **active** `intent_id` and `authorized_intent_revision`; it is the only Decision family whose scope and Policy structure come from that exact Intent revision. Other kinds may carry an originating Intent as causal provenance; a measurable roadmap Decision requires one so later impact work can explain which objective triggered the choice. That causal id does not scope the brand-wide Strategy, enumerate the Intents it covers, copy exact-Intent preauthorization, or become a future active-status gate. A terminal accepted Intent remains valid historical provenance, while an agent-authored draft is never valid Decision authority or origin. The Better Auth role matrix remains organization-wide code. A `policy_restriction` may only tighten one brand; organization-wide Decision Objects are deferred rather than simulated by duplicated or cross-brand rows.

### D3 — Active Decisions are human-authored; agents recommend

V1 `recordDecision` is a product mutation under ADR-001's current non-viewer role matrix. It reloads the exact current Better Auth Member, materializes that User's global human Actor, and records that Actor as the producing Action. Task-bound CMO and specialist sessions may produce Evidence, reports, open questions, and typed Decision recommendations under their durable output contracts, but they cannot call `recordDecision` or impersonate the human who initiated a conversation. The top-level interactive CMO may instead call the read-only `present_decision_recommendation` typed presentation tool; it has no task origin, cannot call `recommend_decision`, and creates no Object or Action before the human click.

No pending-Decision table or generic Proposal Object is added. A **durable** recommendation is an ordinary immutable `report` Object with closed subtype `decision_recommendation`, produced only by task-bound durable work; it contains the proposed `DecisionContent`, nullable origin Intent id, rationale, the exact head id observed by the agent, and nullable `expected_intent_revision`. For an `intent_preauthorization`, trusted code derives **both** non-null `origin_intent_id` and `expected_intent_revision` from the same producing task's exact Intent snapshot and rejects any model-proposed mismatch; those fields are null/ordinary causal provenance as appropriate for other Decision kinds. An origin-free or Plan-routed task therefore cannot recommend a preauthorization. A report produced by `decision-reconsideration` is narrower: its origin Intent id is non-null, is derived from the Decision's producing Action, and cannot be changed or omitted by the CMO. `TaskCompletion` references that report Object id rather than duplicating the proposal. A report-backed chat card or console button loads that exact Object and its source task snapshot, shows the exact revision and Intent facts, and invokes an authenticated Server Action; trusted code revalidates content, origin and revision against that same snapshot before calling `recordDecision` with its source report id, expected head and expected Intent revision.

`present_decision_recommendation` is mounted only in the authenticated top-level CMO wrapper. Its closed model input is `{ proposedDecision, originIntentId, rationale }`; model/browser input cannot supply brand, Member, current head, source report, expected Intent revision or request id. The tool requires the exact application conversation/session binding, no parent session and a current non-viewer human Member, validates the proposed Decision and same-brand non-draft causal Intent, reloads the exact current Decision head, and returns one closed canonical card payload containing the validated proposal/origin/rationale, derived `expectedHeadId` plus its display summary, explicit `sourceRecommendationId: null`, and a `requestId` deterministically derived from `(conversation_id, session_id, turn_id, digest(canonical card))`. For an `intent_preauthorization` it additionally requires the Intent be active, derives `expectedIntentRevision` from that locked/read canonical row, and includes the exact statement, criteria and constraints in the rendered card; for every other kind that field is null. Eve `callId` is telemetry only and is excluded from that identity. The tool is read-only: its `action.result` may be rendered or replayed as conversation state, but it is not durable product evidence or agent-authored canonical provenance.

The card button submits that exact returned payload to the authenticated human boundary; it never reconstructs fields from assistant text or generates a fresh request id. Same-card repeated tool calls in the same turn and UI double-click/lost-response retry therefore converge, while a changed proposal produces another id. If the Decision head changes after presentation but before the first click, expected-head CAS returns `stale_head` with no write and the UI must request a fresh card. On a preauthorization receipt miss, any Intent status/revision drift returns `stale_intent` with no write; an exact successful replay remains historical, and the server never silently rebases a new click onto the newer revision. Schema validation, current Member/Actor resolution, Policy and the Action request hash remain identical to the report-backed path. The human click authorizes that exact typed mutation rather than retroactively turning the card into a Report. For `intent_preauthorization`, either renderer must show the exact Intent revision plus statement, criteria, constraints, registered key/effect and a notice that any later refinement requires reconfirmation; a stale card is disabled and refreshed. For `roadmap_input:strategy`, either renderer must show the proposed content, label the Intent as causal provenance, identify the exact current Strategy head/summary that will be replaced (or create-only null), state that the scope is the entire brand and that a global Plan rebuild follows, and state that already accepted/running work and commitments are not cancelled. A renderer omitting any required facts cannot expose the action button. The Strategy origin may now be terminal; actionability depends on the exact Decision head, not on reopening or keeping that historical Intent active. Only the subsequent human action creates or supersedes the canonical Decision. This preserves “never overwrite a human” without inventing another task or execution lifecycle.

### D4 — `recordDecision` atomically writes the head and its required work

The trusted operation accepts `brandId`, nullable `originIntentId`, nullable same-brand `sourceRecommendationId`, exact `expectedHeadId` (including explicit `null` for create-only), nullable positive `expectedIntentRevision`, typed content, a stable `requestId`, and human rationale. Trusted code derives `operation_key = decision-record:<user_id>:<request_id>` and a canonical `request_hash` over the complete intended mutation, including the nullable source report, expected head, origin Intent, expected Intent revision, typed content, and rationale. When a source report is present, its closed recommendation content, origin Intent and expected Intent revision must agree exactly with the request. When it is null, the operation claims no durable recommendation provenance: the validated content is simply the exact mutation selected by the current human, whether entered directly or submitted from an ephemeral CMO card/form. `intent_preauthorization` requires a non-null exact same-brand **active** Intent and a non-null expected revision pinned by the trusted presentation/form read; every other kind requires `expectedIntentRevision = null`. The server compares rather than replaces that value and uses the Intent-bound Policy branch. A measurable `roadmap_input` also requires a non-null same-brand origin, but only as causal provenance: `active`, `settled`, and `abandoned` accepted Intents are valid, while `draft` is rejected. Every roadmap Strategy remains a brand-wide mutation evaluated with `structure_level = null`, no exact-Intent preauthorization set, and current brand restrictions. A terminal causal origin therefore does not make a reconsideration report obsolete, reopen the Intent, or turn the replacement into an Intent-scoped Decision. Other administrative brand Decisions may omit origin. Actor, organization role and brand scope come from `BrainWriteContext`, not request fields.

Before the transaction, code validates the closed Decision shape and performs no provider I/O. In one transaction `recordDecision` resolves the brand, locks the exact current Better Auth Member `FOR SHARE`, and calls `ensureHumanActor` in the common order. It then acquires the transaction-scoped advisory lock derived from `(brand_id, operation_key)` and looks up the Action receipt. Same key and current hash returns the historical Decision and every deterministically derived task created with it without rechecking the now-current head or replaying Policy; same key with another hash fails. For an already-persisted preauthorization receipt whose typed Action payload predates `authorized_intent_revision`, the compatibility reader permits that missing field only as legacy historical data. If its stored hash differs from the current request hash, trusted code computes the one previous canonical hash shape—identical request semantics but without `expectedIntentRevision`; an exact match returns only that historical receipt/Decision before current guards, while mismatch still conflicts. It does not derive a revision, make the old Decision applicable, or rewrite the Action. Reconfirmation always uses a new request id/current hash and writes the explicit revision metadata. The advisory lock makes concurrent first calls converge on that lookup; the partial unique index remains the database backstop. Only when no replay row exists does it continue:

1. when `originIntentId` is non-null, locks and validates that exact same-brand Intent; for `intent_preauthorization`, requires `active` and `revision = expectedIntentRevision`, otherwise rejects `draft` while allowing accepted historical states; separately enforces non-null for every registered Decision kind that requires causal Intent provenance; then locks the derived Decision head and reads/validates the immutable source recommendation and every referenced same-brand `metric_observation` Evidence Object. This Intent-row-first order is shared with refinement and new Intent-bound task insertion, so record-vs-refine and record-vs-insert are serializable before the Decision-head lock;
...
3. allocates the new Object id and appends the human `decision_recorded` Action with that operation key/hash; for `intent_preauthorization`, its typed payload and Policy snapshot also freeze the trusted `authorized_intent_revision = expectedIntentRevision`, while other Decision kinds carry no such fact;
...
- A new Intent-bound task first locks and reads the active canonical Intent revision, then selects only the current active `intent_preauthorization` head for each applicable key with `content.authorized = true`, whose producing Action has that exact `intent_id` **and** `authorized_intent_revision` equal to the locked revision. Missing/legacy revision metadata and a positive head bound to an older revision are inapplicable; selection never falls back to a superseded older positive. An active `authorized = false` head is the durable revocation and never enters the snapshot or raises structure. Only positively authorized Object ids, their bound revision and minimal typed facts enter `intent_snapshot.preauthorizing_decisions`. Refinement does not mutate or supersede the current Decision head: it makes a positive head revision-stale for future work until a human records a replacement against the same revision-agnostic singleton key and exact old head. Existing task snapshots remain unchanged.
...
- a legacy preauthorization receipt cannot parse or exact-replay because its Action/hash predates the revision field; the single previous-hash fallback accepts any semantic difference, performs current guards/writes, or makes that historical Decision applicable to new work; or reconfirmation reuses the old request id instead of writing a new current-format receipt;
- an agent/system Actor records or supersedes an active v1 Decision, an ephemeral CMO card is presented as canonical/durable before the human action, or the interactive CMO can persist it through `recommend_decision` without a task;
- a durable Decision recommendation cannot be recovered by exact report Object id, is duplicated in TaskCompletion, or is accepted after its observed head changed; the reconsideration kind lacks its closed recommended/obsolete result, a reconsideration report omits/changes its immutable causal origin Intent or ids, the human boundary drops that origin, or the report remains actionable after the Decision head became ineligible;
- an interactive recommendation requires a fake task/Report before its button works, is produced without the closed top-level presentation tool, lets model/browser input supply head/source/request identity, changes any returned field at the button, submits `sourceRecommendationId != null`, includes `callId` in identity, fails to converge for the same canonical card/turn, or causes `recordDecision` to claim durable agent provenance that does not exist;
...
- an Intent preauthorization is selected through JSON alone, crosses brand/Intent scope, has `authorized = false`, lacks trusted producing-Action revision metadata, differs from the locked current Intent revision, falls back to a superseded positive head, or lets a roadmap/model/restriction Decision leak into `preauthorizing_decisions`;
- a preauthorization card/report/direct form omits the exact expected Intent revision or facts shown to the human, lets the model choose that revision, silently rebases a rev-N click onto rev N+1, or writes after revision drift instead of returning `stale_intent`; an exact successful rev-N replay after refinement fails to return its historical receipt, or reconfirmation on N+1 cannot supersede the same logical head while preserving the old Decision/task snapshots;
- a task-bound preauthorization recommendation derives `origin_intent_id` and `expected_intent_revision` from different sources, accepts either as model-authored, or lets a task on Intent A authorize Intent B merely because both currently have the same revision number;
- a legacy non-preauthorization report cannot be read as null revision, or a legacy preauthorization report remains actionable without deriving one exact matching Intent/revision pair from its immutable source task snapshot;
- record-vs-refine or task-insert-vs-refine can observe a torn revision/preauthorization set instead of serializing on the Intent row; a record that wins before refinement cannot become safely inapplicable afterward, or a refinement that wins first still lets the stale record commit;
