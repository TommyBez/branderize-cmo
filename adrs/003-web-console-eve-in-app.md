...
1. **Approval inbox** — pending boundary Actions awaiting human judgment; the main surface
...
- **No intelligence in the console routes.** Routes read projections and write only intents and approvals through `packages/brain`. An app route that calls an enrichment API or makes a judgment is treated as a bug (the CRM's rule).
