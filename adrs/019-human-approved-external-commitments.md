...
The commitment task is distinct from the parent agent task that prepared it. The parent may later succeed or fail without changing a valid pending commitment. `parent_task_id` preserves causation; it does not make the child transactional with the parent result.
...
1. locks or compares-and-swaps the exact task revision still in `awaiting_approval`;
2. validates the registered schema and canonical payload hash;
3. derives the worker, effect signature, scope, external-effect cost bound, preview manifest, connector operation, receipt schema, concurrency policy, optional conflict key, and billing branch from the final payload and closed registry entry;
4. evaluates Policy against the current human Actor and role;
5. appends an Approval Action containing the task id and revision, actor, final payload hash, effect signature, `policy_snapshot`, authorized external-effect cost bound, and the resolved billing snapshot (`non_billable` or fixed `price_key`, `pricing_version`, currency, and unit amount);
6. stores `approval_action_id`, `approved_at` from the same transaction timestamp, and the derived `commitment_conflict_key`, if any, on the task and changes it to `queued`.
...
A committed Approval Action is a durable grant, not a promise to re-evaluate the approver later. The approver's membership and role are authoritative at the click and are preserved in `policy_snapshot`; later removal from the brand or degradation to a weaker role does not invalidate an already-`queued` task. Claim validates the persisted task/revision/hash/Approval relationship but deliberately does not reload the former approver's current membership. Offboarding creates no task watcher and performs no implicit cancellation. Likewise, removing the human who originally proposed or edited a task still in `awaiting_approval` does not cancel that task: another currently authorized member may approve it, while the removed human can no longer perform the click.
...
Delivery and wake-up may retry while the task is still `queued`, because no provider operation has started. The responsible root completes every deterministic preflight that can reject the operation without contacting the provider while the row is still `queued`. A temporarily missing runtime capability, such as a disconnected provider, leaves the row `queued` with a visible diagnostic; autonomous AI-credit balance is explicitly **not** such a capability for this lane. An expired deadline changes the row directly to `expired`, and a payload or registered contract that can no longer be executed safely changes it to `needs_regeneration`. None of those paths calls the provider. Neither `approveTask`, this preflight, nor the claim reads the agent credit balance. The claim transaction then verifies the integrity of the persisted Approval grant, exact final task revision and hash, supported kind, recorded effect signature and connector operation, current registry compatibility, and deadline before atomically changing `queued -> running`. It does **not** rerun Policy against the current Actor, role, or membership. That claim is the point of no return and happens immediately before external execution. After it commits, the handler makes **at most one provider call**:
...
After that separately bounded classification, approved human commitments receive the first claim opportunities in the root's bounded drain batch. They are ordered by earliest `execute_before` with null deadlines last, then by task `approved_at` and id. `approved_at` is lifecycle data copied from the Approval transaction, not caller-authored priority; the transaction guarantees that it agrees with the linked Action. Only successful `queued -> running` claims consume execution capacity; an expired, incompatible, or temporarily blocked candidate is terminalized or diagnosed according to this ADR and scanning continues until the configured candidate/runtime bound. Any capacity left flows to retry-safe direct/automatic work and then agent work. This preference neither preempts an accepted run nor promises that a task approved after the claim snapshot enters the current batch, and its batch bound is per invocation rather than a root-wide concurrency semaphore.

Every provider handler has an explicit `PROVIDER_HTTP_TIMEOUT`, and the root route exports an explicitly configured Vercel `maxDuration`, named `ROOT_MAX_DURATION` in application configuration and tests. Configuration and contract tests require `PROVIDER_HTTP_TIMEOUT < ROOT_MAX_DURATION < STALE_AFTER`, with a deliberate safety margin before the stale boundary. `waitUntil` lets the drain continue after the `202` acknowledgment but remains bounded by the same Function lifetime; it is not a durable worker. This ordering ensures the stale classifier cannot race an invocation that Vercel is still allowed to run. No pre-call marker is stored: a timestamp written before HTTP would not prove that any byte reached the provider, and safely returning its row to `queued` would require a fenced claim generation that v1 deliberately does not add.

A user who wants to try again after `failed` or `outcome_unknown` performs another explicit approval. The application creates a new task and Approval Action linked by `retry_of_task_id`; it never reopens the old row. The UI warns that an `outcome_unknown` retry may duplicate an effect.

The brand's autonomous AI-credit balance is not a claim precondition for this lane. A `direct/human` task whose Approval Action is valid proceeds even when that balance is zero because it starts no model session. If its registered boundary operation is billable, exactly one `action_charge` keyed by `action_id` is created **only when the task reaches `succeeded` with a validated stable receipt**; consumption beyond the included pool follows the paid-overage path. `failed`, `outcome_unknown`, `expired`, `cancelled`, and `needs_regeneration` are not charged. Agent-session budget enforcement remains separate and cannot be placed in the shared claim predicate.
...
Revision and regeneration are explicit. A replacement task carries `supersedes_task_id`, and the old pending task becomes `superseded`; a stale click cannot approve it. A human dismissal changes the task to terminal `dismissed` and appends a Dismissal Action with actor, rationale, and timestamp. Exact recreation of the same `(brand_id, kind, canonical payload hash)` is blocked until a human records an explicit Reopen Action. `prepareCommitment` serializes this check and insert with a transaction-scoped advisory lock derived from that tuple, then reads the latest Dismissal/Reopen fact before inserting. Reopen acquires the same tuple lock before appending its Action. This is exact dismissal memory, not a fuzzy semantic fingerprint and not a new alias table.
...
- a claim re-evaluates the former approver's current membership or role, offboarding silently revokes an already committed Approval Action, or administrative cancellation can win after `running`;
...
- a root has no stale-run classifier for its human commitments, classifies them before `STALE_AFTER`, allows `PROVIDER_HTTP_TIMEOUT >= ROOT_MAX_DURATION` or `ROOT_MAX_DURATION >= STALE_AFTER`, or assumes `waitUntil` outlives the Function limit;
- a claimable approved commitment sits behind agent/internal work selected in the same batch snapshot, human ordering ignores an earlier `execute_before`, or implementing this preference requires caller-authored/persisted generic priority;
- a stale backlog can consume an unbounded drain, a non-claiming candidate consumes `DRAIN_BATCH`, `approved_at` can disagree with its Approval transaction, or a per-invocation batch is represented as a false global concurrency guarantee;
...
- Membership and role changes do not retroactively revoke a committed Approval Action; an explicit pre-claim cancellation is the revocation mechanism.
