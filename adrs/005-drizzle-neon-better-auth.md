...
- **Authentication is Better Auth** (already recorded in ADR-001: passwordless email OTP and organization-wide membership). Better Auth ships a first-party Drizzle adapter, so one ORM covers both the auth tables and the domain tables. React Email renders the OTP message and Resend delivers it outside guarded local development. `Member(organization_id, user_id)` is the only v1 human membership relation; there is no Better Auth Team or `brand_memberships` table.
...
- **Local development**: every developer uses a dedicated persistent branch in
  the non-production Neon project. Runtime code receives the branch's pooled
  `DATABASE_URL`; migrations alone receive its direct `DIRECT_DATABASE_URL`.
  Docker is not a development database option and remains confined to automated
