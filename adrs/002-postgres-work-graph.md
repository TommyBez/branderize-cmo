...
- **A single write path**: `packages/brain` exposes typed functions that evaluate Policy, persist the `policy_snapshot`, and write Action + Object in one transaction. Agent tools and console routes all go through it. V1 `recordDecision` is human-only; when a roadmap Decision is measurable, that same transaction also creates its exact one-shot impact-verification task
