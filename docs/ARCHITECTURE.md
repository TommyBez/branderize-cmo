...
The two declarations share one registry entry and differ in three declared deltas (ADR-015): the subagent returns through eve's `outputSchema` task mode (the return contract is framework-enforced), the root returns via brain writes and task completion, and each gets a short instruction addendum for its stance. eve's built-in self-copy `agent` tool is enabled uniformly: a copy is intra-session parallelism inside an already-dispatched, already-budgeted session — the queue governs parallelism across work items, self-copies parallelize within one. eve's bounds hold everywhere (copies cannot recurse; the shared sandbox requires non-overlapping write scopes), and specialist copies are leaves by construction since no nested subagents are authored. Capability gating never disables a whole agent: task kinds declare `requires[]` (checked at lease) and capability-bound tools are dynamically omitted per session, with eve's OAuth challenge doubling as the connect link.
