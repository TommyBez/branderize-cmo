...
Dependencies explain sequencing but do not gate task claims. Later work advances the same immutable Plan through ADR-021's `advance-marketing-plan`, whose wave Action records another exact route mapping without creating a Plan version. Terminal mapped work and qualifying same-origin descendants may only ring a best-effort wake-up bell; they do not prove Move readiness or completion. A terminal provider-outcome Verification is a distinct bell: it stays origin-free, derives its Plan ancestry through the originating commitment, and uses its own Verification-scoped replay identity so it cannot be confused with the earlier command-acceptance signal. `pending` never rings it. **Ricontrolla** recovers a missed/absorbed wake-up, while **Avvia comunque** overrides only the CMO's readiness judgment for one included Move.
...
An accepted external command may create a same-root origin-free provider-outcome poll only when its registry contract supplies durable lookup and bounded cadence; ADR-019 owns that deterministic read lifecycle. `pending` creates only the next poll. After a terminal provider Verification commits, ADR-021 may best-effort wake the current Plan by resolving its immutable Result Action back to the originating Plan-derived commitment; the poll never inherits Plan origin or authority, and failure to enqueue changes no observed fact. Marketing impact is separate: ADR-020's measurable Decision creates one future origin-free Growth task whose model may choose any available measurement read tools and records a typed Verification Action. Intent acceptance remains the independent exact-revision Verification owned by ADR-012. None of these mechanisms silently reopens an Intent, retries an ambiguous external commitment, or turns a Plan dependency into a dispatcher predicate.
