  ...
  test('the four Phase 0 mandatory journeys cross browser, boundaries, and PostgreSQL', async ({
    ...
    try {
      ...
      if (onboarding === undefined) {
        throw new Error(
          'Onboarding did not commit its canonical database proof'
        )
      }
      ...
      await expect
        .poll(async () => {
          const result =
            await databasePool.query<ConversationCheckpointProofRow>(
              `SELECT
      ...
      await ownerPage.goto(
        `/brands/${brandId}/work/${requestedTaskProof.taskId}`
      )
      await expect(
      ...
      const readRawStreamBoundary = async (
        ...
        try {
          ...
          responseHeaders = {
            ...
            'x-eve-stream-version': response.headers.get(
          ...
          if (status === 200) {
            ...
            if (
              ...
              throw new Error(
                `${label} proxy stream omitted a valid tail index`
              )
            }
        ...
        try {
          ...
          if (rawTailIndex !== null) {
            if (!STREAM_TAIL_INDEX_PATTERN.test(rawTailIndex)) {
              throw new Error(
                `${label} direct CMO stream returned invalid tail`
              )
            }
            tailIndex = Number(rawTailIndex)
            if (!Number.isSafeInteger(tailIndex) || tailIndex < -1) {
      ...
      await expect(
        ownerPage.getByText(
          `Product Marketer asks: ${PRODUCT_MARKETER_QUESTION}`
        )
      ).toBeVisible({ timeout: 30_000 })
      await expect(ownerPage.locator('.cmo-status')).toContainText('Ready')
      ...
      await expect(
        ownerPage.getByText(
          `This turn attests task ${blockedTaskProof.taskId}.`
        )
      ).toBeVisible()
      ...
      try {
        await Promise.all(
          contextsToClose.map(async (context) => context.close())
        )
      } finally {
        await cleanTestData(registry)
      }
...
test.describe('Phase 1 console journeys', () => {
  test('prove proposals, approvals, work, and connections', async ({
    browser,
  }) => {
    test.setTimeout(180_000)
    const suffix = randomUUID().slice(0, 8)
    ...
    try {
      ...
      await expect(
        viewerPage.getByRole('button', { name: 'Adopt' })
      ).toHaveCount(0)
      await expect(
      ...
      await memberAdoptPage.goto(
        `/brands/${brandId}/intent/proposals/${draftId}`
      )
      await Promise.all([
        memberPage.getByRole('button', { name: 'Adopt' }).click(),
        memberAdoptPage.getByRole('button', { name: 'Adopt' }).click(),
      ])
      const adoptSettled = (page: Page): Locator =>
        page
          .locator('.form-feedback')
          .or(
            page.getByRole('heading', { name: PRIVATE_ROUTE_BOUNDARY_PATTERN })
          )
      await expect(adoptSettled(memberPage)).toBeVisible()
      await expect(adoptSettled(memberAdoptPage)).toBeVisible()
      const adoptFeedback = [
        ...(await memberPage.locator('.form-feedback').allTextContents()),
        ...(await memberAdoptPage.locator('.form-feedback').allTextContents()),
      ]
      ...
      expect(
        adopted.rows[0],
        `adopt read-back after both Adopt server actions settled; form feedback: ${adoptFeedback.join(' | ') || '(none)'}`
      ).toMatchObject({
        actionCount: 1,
        status: 'active',
      })
      ...
      await expect(
        ownerPage
          .getByText('no active Notion connection')
          .filter({ visible: true })
      ).toBeVisible()
      await expect(
        ownerPage.locator('article').getByText(TOKEN_WORD_PATTERN)
      ).toHaveCount(0)
      await ownerPage
        .locator('article')
      ...
      await ownerPage
        ...
        .click()
      await expect(
        ownerPage.getByText('Acme Notion workspace').filter({ visible: true })
      ).toBeVisible()
      await expect(
        ownerPage.getByText('Scopes: read, write').filter({ visible: true })
      ).toBeVisible()
      ...
      await expect(
        ownerPage.getByText(notionTitle).filter({ visible: true })
      ).toBeVisible()
      await expect(
        ownerPage
          .locator('article')
      ...
      await ownerPage
        ...
        .getByRole('button', { name: 'Approve Notion page-create' })
        .click()
      await expect(ownerPage.getByText(notionTitle)).toHaveCount(0)
      ...
      await expect(
        ownerPage.getByText('commitment approved').filter({ visible: true })
      ).toBeVisible()
      await expect(
        ownerPage.getByText('Result', { exact: true }).filter({ visible: true })
      ).toBeVisible()
      await expect(
        ownerPage.getByText('commitment result').filter({ visible: true })
      ).toBeVisible()
      ...
      await ownerPage
        ...
        .click()
      await expect(ownerPage.getByText(`Dismiss ${suffix}`)).toHaveCount(0)
      ...
      await expect(
        ownerPage.getByText('Commitment reopened.').filter({ visible: true })
      ).toBeVisible()
      expect(
        await latestDismissalFact({
          brandId,
      ...
      await expect(
        ownerPage.getByText('Content · succeeded').filter({ visible: true })
      ).toBeVisible()
      await expect(
        ownerPage.getByText('Distribution · failed').filter({ visible: true })
      ).toBeVisible()
      await expect(
        ownerPage.getByText('SEO · cancelled').filter({ visible: true })
      ).toBeVisible()
      await expect(
        ownerPage.getByText('Notion page · succeeded').filter({ visible: true })
      ).toBeVisible()
      await expect(
        ownerPage
          .getByRole('link')
          .filter({ hasText: contentTaskId.slice(0, 8) })
      ).toBeVisible()
      await expect(
        ownerPage
          .getByRole('link')
