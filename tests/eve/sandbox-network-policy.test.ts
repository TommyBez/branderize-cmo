...
const backendFactories = {
  docker: vi.fn((options: unknown) => ({ kind: 'docker', options })),
  vercel: vi.fn((options: unknown) => ({ kind: 'vercel', options })),
}

const UNSAFE_SANDBOX_BACKEND_PATTERN =
  /defaultBackend|just-?bash|microsandbox/iu
...
describe('Eve sandbox network policy', () => {
  it('pins every owner to a deny-all Docker or Vercel backend without fallback', async () => {
    ...
    for (const { name, source } of ownerSources) {
      expect(source, name).toContain("from 'eve/sandbox/docker'")
      expect(source, name).toContain("from 'eve/sandbox/vercel'")
    ...
    vi.doMock(requireFromAgent.resolve('eve/sandbox/docker'), () => ({
      docker: backendFactories.docker,
    }))
    vi.doMock(requireFromAgent.resolve('eve/sandbox/vercel'), () => ({
      vercel: backendFactories.vercel,
    }))

    const expectBackendBranch = async (branch: {
      readonly backend: 'docker' | 'vercel'
      readonly vercelEnvironment: string
    }): Promise<void> => {
      vi.resetModules()
      vi.clearAllMocks()
      vi.stubEnv('VERCEL', branch.vercelEnvironment)

      const definitions = await Promise.all(
        sandboxOwners.map(async ({ load }) => (await load()).default)
      )

      expect(backendFactories.docker).not.toHaveBeenCalled()
      expect(backendFactories.vercel).not.toHaveBeenCalled()

      const selectedFactory = backendFactories[branch.backend]
      const unselectedFactory =
        branch.backend === 'docker'
          ? backendFactories.vercel
          : backendFactories.docker
      ...
      expect(selectedFactory).toHaveBeenCalledTimes(sandboxOwners.length)
      ...
      for (const [options] of selectedFactory.mock.calls) {
        expect(options).toEqual({ networkPolicy: 'deny-all' })
      }
    }

    await expectBackendBranch({ backend: 'docker', vercelEnvironment: '' })
    await expectBackendBranch({ backend: 'vercel', vercelEnvironment: '1' })
  })
})
