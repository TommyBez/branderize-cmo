...
const loadProducedObjectId = async ({
  ...
  if (result === undefined) {
    throw new Error('The Distribution task has no trusted channel plan output')
  }
  return result.id
}
...
const completionFromInput = async (
  ...
  database: Database
): Promise<DistributionChannelPlanCompletion> => {
  if (input.status !== 'completed') {
    return distributionChannelPlanCompletionSchema.parse({
...
export default defineTool({
  description:
    'Stage the authoritative terminal Distribution completion. Completed work requires a prior save_channel_plan receipt; partial or blocked work records questions without writing an Object.',
  ...
  outputSchema: distributionChannelPlanCompletionSchema,
})
