...
export const ROOT_AGENT_KEY = 'product-marketer' satisfies AgentKey

const rootDefinition = agentRegistry[ROOT_AGENT_KEY]
const rootManifest = getRootManifest(ROOT_AGENT_KEY)

if (rootManifest.functional !== (rootDefinition.status === 'functional')) {
  throw new Error(
    'The Product Marketer root manifest does not match the agent registry'
  )
}
