...
const loadProducedObjectId = async ({
  ...
  if (result === undefined) {
    throw new Error('The SEO Discovery task has no trusted evidence output')
  }
  return result.id
}
...
const completionFromInput = async (
  ...
  database: Database
): Promise<SeoDiscoveryOpportunityCompletion> => {
  if (input.status !== 'completed') {
    return seoDiscoveryOpportunityCompletionSchema.parse({
  ...
  return seoDiscoveryOpportunityCompletionSchema.parse({
    ...
    result: { evidenceObjectId: objectId, outcome: 'report' },
    status: 'completed',
    summary: input.summary,
  })
}
...
export default defineTool({
  description:
    'Stage the authoritative terminal SEO Discovery completion. Completed work requires a prior save_seo_opportunity receipt; partial or blocked work records questions without writing an Object.',
  ...
  outputSchema: seoDiscoveryOpportunityCompletionSchema,
})
